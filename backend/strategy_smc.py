...
def evaluate(symbol: str, htf: pd.DataFrame, dtf: pd.DataFrame, ltf: pd.DataFrame,
    ...
    # SHORT — classic SMC on a per-symbol bearish alignment. This is the validated
    # short edge (PF 1.62) and fires in ANY BTC regime, not just BEAR. Checked
    # first; a bearish-aligned symbol won't have a bullish Phoenix setup anyway.
    if config.SMC_ALLOW_SHORT and _direction(htf, dtf, ltf) == "short":
        if not (gate and cpi_bias == "BULLISH"):   # don't short a falling-inflation backdrop
            sig = evaluate_smc_machine(symbol, htf, dtf, ltf, regime)
            if sig is not None:
                return sig
    # LONG — Phoenix, only in a BULL market regime (matches phoenix_backtester).
    if reg == "BULL" and config.SMC_ALLOW_LONG:
        if gate and cpi_bias == "BEARISH":         # don't long a rising-inflation backdrop
            return None
